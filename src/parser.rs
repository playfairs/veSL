...
impl PngDocument {
    pub fn parse(input: &[u8]) -> Result<Self, Error> {
        ...
        while offset < input.len() {
            ...
            let data_end = data_start.checked_add(data_len).ok_or(Error::Truncated {
            ...
            if input[data_end..crc_end] != crate::chunk::crc32(&[&kind, &data]) {
        ...
        if !saw_iend {
            return Err(Error::MissingIend);
        }
        if offset != input.len() {
            return Err(Error::DataAfterIend);
        }
